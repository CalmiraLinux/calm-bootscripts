...
files: create-dirs 
	install -m ${MODE} calm/init.d/checkfs       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/cleanfs       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/halt          ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/console       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/localnet      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/modules       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/mountfs       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/mountvirtfs   ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/network       ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/rc            ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/reboot        ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/sendsignals   ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/setclock      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/swap          ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/sysctl        ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/sysklogd      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/template      ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/udev          ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/init.d/udev_retry    ${ETCDIR}/rc.d/init.d/
	install -m ${MODE} calm/sbin/ifup            ${SBIN}
	install -m ${MODE} calm/sbin/ifdown          ${SBIN}
	install -m ${CONFMODE} calm/sbin/ifup.8      ${MAN8}
	ln -sf  ifup.8                              ${MAN8}/ifdown.8
	install -m ${MODE}     calm/lib/services/ipv4-static-route  ${SERVICEDIR}
	install -m ${MODE}     calm/lib/services/ipv4-static        ${SERVICEDIR}
	install -m ${CONFMODE} calm/lib/services/init-functions     ${SERVICEDIR}

	if [ ! -f ${ETCDIR}/sysconfig/createfiles ]; then \
          install -m ${CONFMODE} calm/sysconfig/createfiles ${ETCDIR}/sysconfig/ ;\
        fi
	if [ ! -f ${ETCDIR}/sysconfig/modules     ]; then \
          install -m ${CONFMODE} calm/sysconfig/modules     ${ETCDIR}/sysconfig/ ;\
        fi
	if [ ! -f ${ETCDIR}/sysconfig/udev_retry  ]; then \
          install -m ${CONFMODE} calm/sysconfig/udev_retry  ${ETCDIR}/sysconfig/ ;\
        fi
	if [ ! -f ${ETCDIR}/sysconfig/rc.site     ]; then \
          install -m ${CONFMODE} calm/sysconfig/rc.site     ${ETCDIR}/sysconfig/ ;\
        fi
